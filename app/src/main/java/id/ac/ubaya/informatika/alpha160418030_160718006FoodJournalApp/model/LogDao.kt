...
interface LogDao {
    ...
    @Query("SELECT logId, id_user, date, count(foodName) AS foodName, sum(calories) AS calories FROM foodLogs WHERE id_user = :id_user AND strftime('%m',date) = strftime('%m',:date) GROUP BY strftime('%d',date)")
